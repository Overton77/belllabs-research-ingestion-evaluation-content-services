...
def sandbox_manifest(run_root: Path, relative_paths: Iterable[str]) -> Manifest:
    """Build an allowlisted read-only manifest rooted in one ephemeral run."""
    ...
    for relative in sorted(set(relative_paths)):
        ...
        if normalized.is_absolute() or ".." in normalized.parts:
            raise ValueError("sandbox inputs must be normalized run-relative paths")
        path = (run_root / normalized).resolve()
        try:
            path.relative_to(resolved_root)
        except ValueError as error:
            raise ValueError("sandbox inputs must remain under the ephemeral run root") from error
        source = path
        if path.is_dir():
